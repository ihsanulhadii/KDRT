...
import com.example.myapplication.adapter.CounselorListAdapter;
...
public class ActivityListCounselor extends AppCompatActivity {
        ...
        private CounselorListAdapter articleListAdapter;
        private List<CounselorModel> articleModelList = new ArrayList<>();
        ...
        protected void onCreate(Bundle savedInstanceState) {
            ...
            articleListAdapter = new CounselorListAdapter(articleModelList,userList);
                    ...
                    .addOnCompleteListener(task -> {
                        ...
                        Thread adminThread = new Thread(() -> {
                            for (DocumentSnapshot document : task.getResult()) {
                                CounselorModel counselorModel = document.toObject(CounselorModel.class);
                                articleModelList.add(counselorModel);
                                Log.d("adminya1", counselorModel.getName());
                            }

                            // When the for loop is done, post a Runnable to call getAdminList on the main thread
                            mainHandler.post(() -> {
                                getCounselorList(); // Call your getAdminList function here
                            });
                        });
        ...
        private void getCounselorList(){
            for (int i = 0; i < articleModelList.size(); i++) {
                String idAdmin = articleModelList.get(i).getName();
                ...
                public void run() {
                    if(!articleModelList.isEmpty()){
                        ...
                        articleListAdapter.setOnItemClickListener(new ChatRoomAdapter.OnItemClickListener() {
                            @Override
                            public void onItemClick(CounselorModel counselorModel, Admin admin) {
                                Intent intent = new Intent(com.example.myapplication.activity.ActivityListChatRoom.this, ChatActivity.class);
                                intent.putExtra("chatroom",counselorModel);
